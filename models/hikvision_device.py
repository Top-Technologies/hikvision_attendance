...
class HikvisionDevice(models.Model):
    ...
    connection_mode = fields.Selection([
        ...
        ('bridge', 'Through Bridge Server')
    ], string='Connection Mode', default='direct', required=True, tracking=True)
    
    bridge_url = fields.Char(
        string='Bridge Server URL',
        help='URL of the bridge server (e.g., http://your-server-ip:8443 or https://bridge.yourdomain.com:8443)')
    
    bridge_token = fields.Char(
        string='Bridge Auth Token',
        help='Authentication token for secure bridge server access (configured in bridge config.yaml)')
    
    bridge_device_id = fields.Char(
        string='Device ID on Bridge',
        help='Device identifier as configured in bridge server config.yaml (e.g., device-1, office-main, etc.)')
    ...
    def action_test_connection(self):
        ...
        try:
            ...
            if response.status_code == 401:
                raise UserError(_("Authentication failed (401). Please check Device Username and Password."))
    ...
    def action_test_bridge_connection(self):
        """Test connection to bridge server"""
        self.ensure_one()
        
        if self.connection_mode != 'bridge':
            raise UserError(_("Device is not in bridge mode"))
        
        if not self.bridge_url or not self.bridge_token:
            raise UserError(_("Bridge URL and Token are required"))
        
        try:
            response = requests.get(
                f"{self.bridge_url}/api/v1/status",
                headers={'Authorization': f'Bearer {self.bridge_token}'},
                timeout=120
            )
            
            if response.status_code == 401:
                raise UserError(_("Bridge authentication failed. Check your token."))
            
            response.raise_for_status()
            data = response.json()
            
            # Check if our device is configured
            devices = data.get('devices', {})
            if self.bridge_device_id and self.bridge_device_id not in devices:
                raise UserError(_(
                    f"Device '{self.bridge_device_id}' not found on bridge server.\n"
                    f"Available devices: {', '.join(devices.keys())}"
                ))
            ...
            return {
                ...
                'tag': 'display_notification',
                'params': {
                    'title': _('Bridge Connection Successful'),
                    'message': _("Connected to bridge server successfully!"),
                    'type': 'success',
                }
            }
            
        except requests.exceptions.ConnectionError:
            raise UserError(_("Cannot connect to bridge server. Check URL and network."))
        except Exception as e:
            raise UserError(_("Bridge connection failed: %s") % str(e))
    
    def _fetch_logs_via_bridge(self, start_date, end_date):
        """Fetch logs through bridge server"""
        self.ensure_one()
        
        if not self.bridge_url or not self.bridge_token or not self.bridge_device_id:
            raise UserError(_("Bridge configuration incomplete"))
        ...
        try:
            # Submit command to bridge
            response = requests.post(
                f"{self.bridge_url}/api/v1/commands",
                headers={
                    'Authorization': f'Bearer {self.bridge_token}',
                    'Content-Type': 'application/json'
                },
                json={
                    'device_id': self.bridge_device_id,
            ...
            _logger.info(f"Bridge command submitted: {command_id}")
            ...
            while time.time() - start_time < max_wait:
                result_response = requests.get(
                    f"{self.bridge_url}/api/v1/commands/{command_id}",
                    headers={'Authorization': f'Bearer {self.bridge_token}'},
                    timeout=120
                )
                ...
                if status == 'completed':
                    ...
                    if not result.get('success'):
                        raise UserError(f"Bridge operation failed: {result.get('error')}")
                    
                    events = result.get('events', [])
                    
                    # Process events (same as direct mode)
                    imported_count = 0
                    skipped_no_employee = []
                    ...
                    message = _('%s events imported from %s to %s via bridge.') % (
                        imported_count, start_dt.date(), end_dt.date()
                    )
                    if skipped_no_employee:
                        message += _(' Skipped %s unmatched employee IDs.') % len(skipped_no_employee)
                    
                    return {
                        'type': 'ir.actions.client',
                        'tag': 'display_notification',
                        'params': {
                            'title': _('Logs Fetched via Bridge'),
                            'message': message,
                            'type': 'success',
                        }
                    }
                    ...
                    raise UserError(f"Bridge command failed: {error}")
                
                time.sleep(2)  # Poll every 2 seconds
            
            raise UserError(_("Bridge operation timeout after 5 minutes"))
            
        except requests.exceptions.RequestException as e:
            raise UserError(f"Bridge communication error: {e}")
    ...
    def action_reboot_device(self):
        ...
        try:
            ...
            if response.status_code in [200, 201]:
                ...
                # Device returns XML error
                raise UserError(_("Reboot failed: %s") % response.text[:200])
        except requests.exceptions.RequestException as e:
    ...
    def action_fetch_logs(self):
        ...
        return self.action_fetch_logs_by_date(start_date, end_date)
    ...
    def action_fetch_logs_by_date(self, start_date, end_date):
        ...
        # First Pass: Find a working strategy
        ...
        if success and active_strategy:
            # Get the batch size from first fetch
            initial_batch_size = len(events) if events else 0
            last_batch_size = initial_batch_size
            current_position = initial_batch_size  # Start from where first batch ended
            
            # Continue fetching as long as we got a full batch
            loop_count = 0
                ...
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        if batch_size > 0:
                            ...
                            # Empty batch means no more events
                            _logger.info("Received empty batch, pagination complete.")
                            break
                    else:
    ...
    def _fetch_single_chunk_with_retry(self, start_date, end_date, chunk_number, total_chunks):
                        ...
                        try:
                            ...
                            if response.status_code == 200:
                                ...
                                if batch_size > 0:
                                    ...
                                    # Empty batch, pagination complete
                                    break
                            elif response.status_code == 401:
        ...
        if not success:
            raise UserError(_("Chunk %s failed after %s retries. Last error: %s") % (chunk_number, max_retries, last_error))
    ...
    def _stream_listener_thread(self, device_id):
        """
        Threaded method to listen to the device event stream.
        
        IMPORTANT: This method creates a new database cursor to safely handle
        concurrent database operations in a background thread. The new cursor
        is essential for thread safety in Odoo's architecture.
        """
                ...
                with requests.get(
                    ...
                    for line in response.iter_lines():
                            ...
                            if line:
                                ...
                                if decoded_line.strip().endswith('}'):
                                    try:
    ...
    def _process_json_event(self, event_data):
        ...
        try:
            ...
            # Parse datetime
            dt = dateutil_parser.parse(date_time)
            if dt.tzinfo:
                dt = dt.astimezone(pytz.UTC).replace(tzinfo=None)
